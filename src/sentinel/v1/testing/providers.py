...
class FakeBlockchainProvider(BlockchainProvider):
    """
    In-memory fake provider with a fluent builder API for test setup.
    ...
    def with_events(self, block_hash: str, events: list[dict[str, Any]]) -> "FakeBlockchainProvider":
        """Add events for a block hash."""
        self.events[block_hash] = events
        return self

    def with_extrinsics(self, block_hash: str, extrinsics: list[dict[str, Any]]) -> "FakeBlockchainProvider":
